...
## 2026-09-16 — imported 8 videos (The Long Spring — January to June 2021, Part One)

- Queue drain, `rotation_index` 2 → **long-spring-2021** (`videos_per_batch` 8, 15 IDs queued, multi-part). No priority videos: `fetch_channel_videos.py` found 0 new uploads (1065 total, unchanged), and no pending video published in the last 14 days. Second `/import` of the day, after the 03:28 UTC tavern-door drain.
- **Headroom fit the slate exactly, no wait needed.** At pre-flight (04:01 UTC) the guard reported **6/15** in the rolling 24h across 1 run (`post_results_20260916_032858.json`, the tavern-door Part One drain 33 min earlier) — **9 of headroom** against an 8-video slate. No trim, no split. The previous run's CHANGELOG note flagged the 13–23 h post-drain band as the risk window; this fetch landed ~0.6 h after that drain, well short of it, and returned 8/8 clean.
- Drift check passed — all 8 slate IDs still `pending`. Transcripts: **8 fetched, 0 permanent failures, 0 transient**.
- Posted to Discourse (category 5, backdated to each video's `published_at`; `post_results_20260916_040612.json`, posted_at 2026-09-16T04:06Z):
  - `NHGisiXq7eg` Waste Knights 2nd Edition - An Apocalyptic Adventure Worth Taking → [t/2198](https://dungeondive.quest/t/2198)
  - `Mtqdsr2BC_A` It's 2021...Let's Talk About the Post-Apocalypse! → [t/2197](https://dungeondive.quest/t/2197)
  - `B_OF2TYLnQw` A Look at Adventure Tactics with a focus on the Character Level Up System → [t/2194](https://dungeondive.quest/t/2194)
  - `cqPVe0nQ1A0` Champions of Hara - A Possible Masterpiece → [t/2199](https://dungeondive.quest/t/2199)
  - `lZj5qfUurLw` Dark Darker Darkest - Taking a Look → [t/2200](https://dungeondive.quest/t/2200)
  - `oUdstwXRu2c` Defenders of the Last Stand - Take a Look → [t/2201](https://dungeondive.quest/t/2201)
  - `ChKL1KyjlZ4` A Look at Darkest Night 2nd Edition → [t/2195](https://dungeondive.quest/t/2195)
  - `DBkInEbzPk0` History Maker Golf - And now for something completely different → [t/2196](https://dungeondive.quest/t/2196)
- Keeper post **"The Long Spring — January to June 2021"** (register A, 381 words of prose; ~690 total with the catalogue) → [t/1170/147](https://dungeondive.quest/t/dungeon-dive-video-archive-update/1170/147). Framing: a crate labelled by season rather than subject, which the Keeper distrusted and then could not argue with — by the fourth panel it had stopped sorting by game and started sorting by weather, because the archivist spent that winter in worlds that had already ended (a wasteland in one box, a shelf of apocalypses opened with the Road Warrior monologue from memory, a mansion of spreading fire whose survivors are named after horror directors and which can be built unwinnable, a ruined southwest, and a country the necromancer has already taken where heroes spend secrecy like coin). The Keeper, which lives in a void between two shelves and has never had a good year, recognises the mood. Counterweight: two panels about growth rather than ruin, and then a closing panel about **golf** — the most argumentative object in the wing, a simulation that discards the walking and the club selection, finishes a hole in under a minute, and is used to prosecute the map board. Exhibit Catalogue runs 13 entries in publish-date order: the 8 new topics plus all 5 queued `related_imported_ids` — 2020 Retrospective (t/2037), State of the Collection 2021 (t/2041), Shadows of Brimstone Part 1 (t/1458), The Dungeon of D (t/1711), Revisiting the Hero's Journey Home (t/2073) — hooks drawn fresh from `archive/posts/`. All 13 hooks ≤15 words.
- Series **continues**: `long-spring-2021` drained 15 → **7**; `last_part` 1, `last_imported` 2026-09-16, `keeper_post` set. Part Two slate is `zVMIZRDSvCc`, `kETEBzJrtSA`, `jh22fly0d-k`, `iCrYuW2ixu8`, `zxwGfKCo6as`, `qJ6pJMN4eiA`, `scIFhQSvvlE`. Entry not completed, so `rotation_index` **incremented 2 → 3**, past the end, **wrapped to 0** — next drain is **modern-shelf** Part Two (6 IDs, vpb 12); `tavern-door` Part Two (6 IDs) sits in slot 1.
- Index now: **955 imported**, 98 pending, 12 no_transcript (1065 total). Archive: 876 transcripts, 955 posts. Dashboard cross-check matched the Keeper sign-off figures.
- Pre-flight: `git pull` already up to date; rate limit 6/15 (9 headroom, exit 0); `test_config.py` OK; `check_integrity.py` WARN (exit 1) on the same long-standing missing-local-transcripts gap (now 79) and unparseable dashboard stats line — 0 errors, not a stop condition.
- Rate budget after this run: **14/15** in the rolling 24h across 2 runs — 6 ageing out ~03:28 UTC and 8 ageing out ~04:06 UTC on 2026-09-17. **Effectively 1 of headroom until then**, so the next `/import` before ~03:28 UTC tomorrow can only take a 1-video priority drop, and even that lands inside the 13–23 h post-drain band where `IpBlocked` has bitten. A full queue drain should wait until after ~04:06 UTC 2026-09-17.
...
- **Waited for the rate window rather than trimming — 103 minutes, past the ~90 guideline.** At pre-flight (23:30 UTC 2026-09-12) the guard reported **10/15** in the rolling 24h — 5 of headroom against an 8-video slate — with the 09-12 nine-video drain (`post_results_20260912_011356.json`) due to age out at 01:13 UTC. Trimming to 5 was allowed by the letter of the rule (strands 3, not fewer) but would have split a one-shot 5 + 3 *and* fetched 22 h after a 9-video drain, inside the 13–23 h band where the IP block has bitten before. The run armed a Monitor poll of the guard (one check a minute), did the fetch-independent prep meanwhile (Keeper voice samples, `related_imported_ids` hooks from `archive/posts/`), and proceeded when the poll fired at 01:14:17 UTC with **0/15** (15 headroom). `import/SKILL.md` step 2 and CLAUDE.md now record ~90 minutes as a guideline, not a cliff, with a ~2 h extension for exactly this case.
- Drift check passed — all 8 slate IDs still `pending`. Transcripts: **8 fetched, 0 permanent failures, 0 transient**, ~24 h after the 09-12 drain.
- Posted to Discourse (category 5, backdated to each video's `published_at`; `post_results_20260913_011936.json`, posted_at 2026-09-13T01:19Z):
  - `4r4Fv5i5zCI` DungeonQuest - Let's Play → [t/2168](https://dungeondive.quest/t/2168)
  - `0irTZK6Z_YI` Dungeon Run (2011) - Introduction → [t/2167](https://dungeondive.quest/t/2167)
  - `bHJfRtPpY6E` Ravage Dungeons of Plunder - Review → [t/2173](https://dungeondive.quest/t/2173)
  - `t4gVVdxtNWE` Altar Quest - A Puzzly Dungeon Dive with Modular Quests → [t/2174](https://dungeondive.quest/t/2174)
  - `CKAGWlmyXbA` Taking a Look at Marching Order - Darkest Dungeon on the Table? → [t/2170](https://dungeondive.quest/t/2170)
  - `QY2ox96a1zA` DungeonQuest - Review - A Dungeon Dive Quick Hit → [t/2172](https://dungeondive.quest/t/2172)
  - `9dFDeUNHViY` Minotaur - arcade dungeon crawling action! → [t/2169](https://dungeondive.quest/t/2169)
  - `Fs8L9V90RDk` DungeonQuest - new expansions coming! → [t/2171](https://dungeondive.quest/t/2171)
- Keeper post **"The Quick Delve — DungeonQuest & the One-Evening Dungeon"** (register A, 360 words of prose; 652 total with the catalogue) → [t/1170/143](https://dungeondive.quest/t/dungeon-dive-video-archive-update/1170/143). Framing: a delivery small enough to arrive under one wing, with a purple whisky bag and a label reading *one evening, no more*; the Keeper, whose day begins at the hour these games insist you leave, unable to understand the hurry; Dragonfire Castle as the wing's single recurring building (1987 and 2014 clothing, the same 15% quoted like grace across five years); the observation that none of these games is about the treasure — every one is about the door (DungeonQuest won by leaving, Dungeon Run's stone-and-chase, Ravage's rulebook that sets *escape* as the tutorial and forgets how, Altar Quest's manual duller than a kitchen tap's, Minotaur's labyrinth scrolling off behind you); and the fan-made merchant now selling an item called the Keeper's Kiss, name unlicensed, pricing fair. Exhibit Catalogue runs 11 entries in publish-date order: the 8 new topics plus all 3 queued `related_imported_ids` — DungeonQuest Top 10 review 2020 (t/1389), FFG/GW Top 10 re-evaluation (t/605), Dragonfire Castle play-through (t/1298) — hooks drawn from `archive/posts/`. All 11 hooks ≤15 words.
- Series **completed**: `quick-delve-dungeonquest` drained 8 → 0 in a single part; removed from `active_series` and appended to `completed_series` (`parts_completed` 1, `total_videos` 8, `completed_date` 2026-09-13, `keeper_post` set). `rotation_index` **stays 1** — removing the entry shifts `small-boxes-big-games` into slot 1 — so the next drain is **small-boxes-big-games** (11 IDs, `videos_per_batch` 11, `one_shot`); `modern-shelf` Part Two (6 IDs) remains in slot 0 for the run after.
- Index now: **929 imported**, 123 pending, 12 no_transcript (1064 total). Archive: 850 transcripts, 929 posts. Dashboard cross-check matched the Keeper sign-off figures.
- Pre-flight: `git pull` fast-forwarded two dashboard files (`docs/content.html`, `docs/insights.html`) from the nightly `/refresh`; rate limit 10/15 at 23:30 UTC and 0/15 at fetch time (see above); `test_config.py` OK; `check_integrity.py` WARN (exit 1) on the same long-standing missing-local-transcripts gap (79) and unparseable dashboard stats line — 0 errors, not a stop condition.
- Rate budget after this run: **8/15** in the rolling 24h, 7 headroom. This run's 8 age out at ~01:19 UTC 2026-09-14. The next slate is an 11-video one-shot: before that stamp it meets 7 of headroom (a 7 + 4 split is allowed but the wait rule applies if the shortfall is within window); after it, the whole 11 fits.
...
- Rate budget after this run: **3/15** in the rolling 24h, 12 headroom. A full 12-video queue drain before ~2026-09-11 12:00 UTC would sit exactly at the cap with zero margin; prefer to let the window roll over before draining `out-there-science-fiction`.
...
- **Correction, same day:** the IP block is ~24h, not ~1h, so this was the rolling-window budget being exceeded, not a YouTube-side change. Run history bears it out: every 12-video drain since July was followed by ≥41h before the next fetch; this was the first return inside 24h (13.3h after the 09-06 drain) and it was refused, while smaller bursts summing to 15 in a day passed on 08-30/31. Guard tightened: `/import` step 2 now runs `check_rate_limit.py --max-videos 12`, so a second run inside a full drain's 24h stops at pre-flight, and headroom is now binding (trim the slate to it; skip if that would strand <3 videos). CLAUDE.md's rate-limit section corrected likewise; the step-2 wording above about headroom being informational was reversed. Expect the block to lift around 2026-09-07 08:00 UTC, 24h after the drain. **Cap then set to 15 at the user's direction** (the most that has passed in a day), so a full drain leaves 3 of headroom rather than 0; `/import` to be rerun later today once the block lifts.
