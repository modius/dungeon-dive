...
2. `python3 scripts/check_rate_limit.py --max-videos 12` — if exit 1, STOP (daily limit reached). **Pass `--max-videos 12`, not the script's default of 20.** The IP block YouTube applies after a burst of caption fetches lasts ~24h, not ~1h, so the rolling 24h count is the binding constraint and a full 12-video drain spends the whole day's budget: every 12-video drain since July 2026 was followed by ≥41h before the next fetch, and the first run to come back inside 24h (2026-09-07, 13h after a 12-drain, one video) was refused with `IpBlocked`. Smaller bursts have summed to 15 in a day without trouble (2026-08-30/31: 4 + 9 + 1 + 5), so the cap is about the burst as much as the count — do not raise `videos_per_batch` above 12 to compensate.
