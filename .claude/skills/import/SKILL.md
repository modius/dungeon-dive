...
2. `python3 scripts/check_rate_limit.py --max-videos 15` — if exit 1, STOP (daily limit reached). **Pass `--max-videos 15`, not the script's default of 20.** The IP block YouTube applies after a burst of caption fetches lasts ~24h, not ~1h, so the rolling 24h count is the binding constraint. 15 is the most that has passed in a day (2026-08-30/31: 4 + 9 + 1 + 5); 20 has never been tested and the one return inside 24h of a full 12-video drain (2026-09-07, one video, 13h later) was refused with `IpBlocked`, so a full drain leaves only 3 of headroom and even that is not guaranteed — though a one-video return 22.8h after a full drain did succeed (2026-09-12), so the block appears to lift somewhere between 13h and 23h. The cap is about the burst as much as the count — do not raise `videos_per_batch` above 12 to compensate.
   - **Headroom is binding.** If the slate is larger than the headroom the script reports, trim it to headroom; the series simply continues as a further part (step 12 handles that whether or not the entry was planned as a one-shot). Exception: if the trim would strand fewer than 3 videos for a later part, skip this cycle instead so the series lands whole. Record the headroom figure in the CHANGELOG pre-flight note as the other runs do.
   - **Before trimming, check when the shortfall clears.** The guard prints every run it is counting with its UTC `posted_at` (the `post_results_*.json` filenames are UTC too); each ages out exactly 24h after that stamp. If enough headroom for the *whole* slate returns within ~90 minutes, wait for it rather than trimming: arm a background poll (Monitor or `run_in_background`, one guard check a minute, exit when headroom ≥ slate size), do the fetch-independent prep meanwhile (read `keeper-posts/`, pull hooks for `related_imported_ids` from `archive/posts/`), then re-run step 2 when it fires and proceed on the fresh figure. Waiting is the safest option the block model allows, and it lands the series whole — splitting a 9-video one-shot into 2 + 7 to save an hour is exactly the outcome the strand-fewer-than-3 exception exists to avoid. (2026-09-12: a 9-video slate met 2 of headroom with the blocking 12-video drain 60 minutes from ageing out; the run waited and drained the full slate. 2026-09-13: an 8-video one-shot met 5 of headroom with the blocking 9-video drain 103 minutes out — past ~90 — and waited anyway, because the alternative was a 5 + 3 split *and* a fetch 22 h after a 9-video drain, inside the 13–23 h band where the block has bitten; the poll fired at 01:14 UTC and the fetch was 8/8.) Treat ~90 minutes as a guideline, not a cliff: when trimming would split a one-shot *and* the trimmed fetch would land inside that 13–23 h band after the last multi-video drain, waiting up to ~2 h is the better call. If the shortfall clears later than that, trim or skip per the rule above.
...
7. `python3 scripts/batch_fetch_transcripts.py -- VIDEO_ID1 VIDEO_ID2 ...`
   ...
   - **Exit code 2** means the script bailed: more than half the batch hit transient failures, so the runner is almost certainly IP-blocked from YouTube. In that case: do NOT mark anything as `no_transcript`, log "transcript fetch blocked — runner IP issue" to CHANGELOG, abort the run cleanly without proceeding to post generation. The queue is unchanged so the next run will retry.
     - **A one-video priority batch trips the >50% rule on any single transient failure**, so exit 2 there is not by itself proof of an IP block. Confirm with at most one retry (pause ~45s first) and, if that fails too, **one** diagnostic `YouTubeTranscriptApi().fetch()` against an already-imported video — not `list()`: listing hits the watch page and can succeed while the caption-download endpoint is blocked (2026-09-07: `list()` returned tracks for both the new upload and a known-good video, `fetch()` raised `IpBlocked` on both). If the known-good fetch also raises `IpBlocked`/`RequestBlocked`, it is the runner IP — abort as above and **stop probing**; every extra request extends the block, which lasts ~24h. The next scheduled run is the retry; do not rerun `/import` the same day.
...
10. Compose a Keeper post in The Keeper's voice — Vancian wry humour, atmospheric, thematic. This is a **teaser**, not an index: the per-video post summaries already do the analytical work. The Keeper's job here is to set atmosphere, frame the batch, and hook readers into clicking through.
    ...
    **B. Priority drop (ad-hoc priority run, fresh uploads):**
    - **Target 100-200 words, hard cap 250 words.** This is an alert, not an essay. The aim is to flag that fresh material has arrived, not to summarise it — the per-video posts already exist for that. **The budget counts only the Keeper's authored prose** — the framing paragraph, the hook phrases, the deeper-stacks lead-in and the closing line. Link titles (the videos' own names, which can run to 17 words each), the stats line and the sign-off are excluded, exactly as register A excludes its Exhibit Catalogue. For calibration: a three-new-plus-three-stacks despatch runs ~300 words in total while its authored prose sits around 175 (2026-09-03, 2026-09-10) — that is on target, not over cap.
...
12. Update `series_queue.json` (skip entirely if this was an ad-hoc priority run — priority videos never touch the queue — or an interactive user override):
    ...
    - **Record progress:** increment `last_part`, set `last_imported` to today's date (YYYY-MM-DD), set `keeper_post` to the URL of the keeper reply just posted.
      - **Leave `one_shot` and `videos_per_batch` alone on a partial drain, even when the entry was planned as a one-shot and a headroom trim has just split it.** Those two fields record `/plan-batch`'s intent; `last_part` records what actually happened, and that is the field to trust. A trimmed one-shot resuming as Part Two with `one_shot: true` still set is the normal outcome of the step-2 trim rule, not drift to repair — and `videos_per_batch` exceeding the number of IDs left is harmless, since step 6 takes *up to* that many. Both fields are dropped anyway when the entry moves to `completed_series`.
